...
loop_num = min([num_images num_samples]);

% placeholder to be deleted
% features_neg = rand(100, (feature_params.template_size / feature_params.hog_cell_size)^2 * 31);
features_neg = zeros(loop_num, (feature_params.template_size / feature_params.hog_cell_size)^2 * 31);
...
%     fprintf('Mining random negatives from %s\n', image_files(i).name)
    ...
    image_hog = vl_hog( img, feature_params.hog_cell_size ); % return a 6x6x31 matrix
